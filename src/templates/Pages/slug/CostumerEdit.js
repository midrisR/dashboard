...
import InputStatus from '../../../components/Form/InputOptions';

const CostumerEdit = ({ data, isSuccess }) => {
	const { form, clearState, handleInput } = useContext(GlobalContext);
	...
	React.useEffect(() => {
		if (isSuccess) {
			handleInput('pemilik', data.pemilik);
			handleInput('toko', data.toko);
			handleInput('telepon', data.telepon);
			handleInput('alamat', data.alamat);
			handleInput('provinsi', data.provinsi);
			handleInput('kabupaten', data.kabupaten);
			handleInput('kecamatan', data.kecamatan);
			handleInput('kelurahan', data.kelurahan);
			handleInput('kode_pos', data.kode_pos);
			handleInput('status', data.status);
			handleInput('foto_pemilik', data.foto_pemilik);
			handleInput('foto_toko', data.foto_toko);
		}
